...
export default class FrameMachineMakerView {
  ...
  getUMLView = () => {
    ...
    // Save button
    const saveButton = this.addSaveButton('UML is successfully saved on disk', 1500);
  ...
  copyImageToCipboard = async () => {
    ...
    toastr.success('UML is successfully copied', '', { ...toastrOptions });
  }

  addSaveButton = (toastrMsg, toastrTimeOut) => {
    ...
    saveButton.onclick = () => {
      ...
      setTimeout(() => {
        toastr.success(toastrMsg, '', { ...toastrOptions });
      }, toastrTimeOut);
    };
  ...
  getCodeMirrorView = (codeMirror) => {
    ...
    clipboardJS.onload = () => {
      ...
      clipboard.on('success', function(e) {
        toastr.success('Output is successfully copied', '', { ...toastrOptions });
      });
    }

    // Save button
    const saveButton = this.addSaveButton('Output is successfully saved on disk', 1000);
    this.element.appendChild(saveButton);
  }
